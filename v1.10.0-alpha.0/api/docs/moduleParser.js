...
const convertStaticMembers = data =>
  data.members.static.map(info => {
    info.name = `static ${info.name}`
    return convertInfoToObject(info)
  })

const convertInfoToObject = (data, isDefault, isClassMember, path) => {
  const isInternalClass = !!data.tags.find(tag => tag.title === 'internal')
  ...
  return {
    ...
    returns: returns,
    import:
      !isClassMember && !isInternalClass
        ? `import ${importTemplate} from 'react-storefront/${path}'`
        : undefined,
    members: type === 'class' ? convertStaticMembers(data) : undefined,
  }
}
...
const parseInfo = (data, results, path, hasNamedExports) => {
  ...
  const exportsObject = convertInfoToObject(data, isDefault, isClassMember, path)
  ...
  // Populate menu items
  if (hasNamedExports && !isClassMember && name) {
    results.items.push(item)
  }

  // When file has one export which is not default, Example: format/price.js
  if (!hasNamedExports && name !== 'default' && name) {
    results.items.push(item)
  }
}

async function moduleParser(filePath, importPath) {
